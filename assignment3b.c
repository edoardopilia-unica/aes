/*
    Assignment 3B: Test Vector Audio Filtering
...
int main()
{
    ...
    //Low Pass Filtering of test vector
	
	//Decomment this section and comment the others to try

    
	int Sample;
	int buffer[N_LP] = {0};
	int i;
	for (i=0; i<N_INPUT_SAMPLES; i++){
		Sample = InputVector[i];
		Sample = FIR_Filter(Sample, buffer, LP, N_LP);  //Filtering
		printf("%d\n", Sample);
	}
