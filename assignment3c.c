/*
    Assignment 3C: Loopback Filtering + Measuring filtering execution time
...
int main()
{
	...
	/*			Step 3: Measuring filter execution time 		    */
	...
	static int histL[N_LP] = {0};
	static int histR[N_LP] = {0};
	...
	while (1) {
		...
		//Reading switch status (SW[0] ON-> Filtering, SW[0] OFF-> No filtering)
		u32 sw_status = Xil_In32(SWI_BASE_ADDR);
		if (sw_status & 0x01) {
		//Filtering (Low-Pass Filtering)
			SampleL = FIR_Filter(SampleL, histL, LP, N_LP);
		    SampleR = FIR_Filter(SampleR, histR, LP, N_LP);
		}
